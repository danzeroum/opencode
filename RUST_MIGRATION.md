...
- ⬜ Next: re-add **TLS** to reqwest for real HTTPS provider calls (**`rustls`+`ring`**, owner-approved; isolated PR so the `cargo-deny` license-allow for `ring` can be iterated alone) + `route/executor.ts` (retry/backoff + redaction, consuming `Status::retryable`); then openai-chat → openai-responses → gemini → bedrock-converse. No public cutover (the runner consumes it; Phase 4).
