...
- ⬜ Next: the reqwest/SSE **transport** (re-adds TLS — **`rustls`+`ring`**, owner-approved; needs a `cargo-deny` license-allow entry for `ring`) + `route/executor.ts` (retry/backoff + redaction); then openai-chat → openai-responses → gemini → bedrock-converse. No public cutover (the runner consumes it; Phase 4).
