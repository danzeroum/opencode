...
- 🟡 `opencode-tools`: `read`/`glob`/`grep` (ripgrep libs), `write`/`edit`/`ls`, `process::run_command`, `git` (shells out to the `git` binary — faithful to `git.ts`, which does the same; no `gix` dep) — landed; PTY next
