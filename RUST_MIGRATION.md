...
- ✅ **Session list cutover** (PR #19): `GET /api/session` (`v2.session.list`, group `session`) → `SessionsResponse` (`{ data: SessionV2Info[], cursor }`), same `SessionStore` with `limit`/`order`/`search`/`project`/`workspace` filters. Opaque keyset cursors are a follow-up (the `cursor` object is returned empty; its fields are optional in the contract). Contract-enforced (`/api/session`); the 400 union (`anyOf[InvalidCursorError, InvalidRequestError]`) reuses the dedup-collapsing normalizer.
