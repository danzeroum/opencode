...
- ⬜ Port the rest of the real `session/runner/*` over this loop (event-bus production → projected history rebuild; overflow/compaction via the `run_turn` transitions), `permission`, `question`, `message`; depends on the `session_context_epoch`/`session_input` tables. Plus the deferred Bedrock SigV4/live transport (Phase 3 follow-up).
