...
- 🟡 Cutovers, one batch per PR into `rust-migration`: **`GET /path`** (`path.get`, group `instance`) done — resolves home/state/config (XDG) + worktree (git via `opencode-tools::git`) + directory; contract-enforced by `openapi-diff`. Next: `config.get`, `find.text`/`find.files` (reuse `grep`/`glob`), `app.agents`/`app.skills`.
