...
>
> **Rotas nativas contrato-enforçadas: 67 paths** (várias com múltiplos métodos) · PRs desta rodada autônoma: #69–#138. **Engine**: runner loop + permission gating (#129–#131) + question flow fim-a-fim (#133–#134) + `prompt_async` (#135) + HITL V1 (#136) + `session.init` (#137). **Fundação V1 `Message`/`Part`** (40 tipos) + `session.message` (#138). CI `rust` verde (#119). **Loading real**: `command`/`skill`/`agent` lêem `.opencode/**.md`. **Escrita real**: session CRUD (`/session`, `/session/{id}`) persiste no DB (#125–#127). **Engine — permission gating completo**: gate bloqueante `StorePermissionGate` (write-class pede aprovação) + store de pendências + listas + `permission.respond` resolve e acorda o run (#129–#130).
...
1. **Engine de execução (runner loop) — ✅ NÚCLEO COMPLETO.** O loop `run_gated` (tool-calling multi-step + emissão de eventos + projeção pro `session_message`) já existia (write-path #80–#82). Nesta rodada o **permission gating ficou real fim-a-fim** (#129–#131): gate bloqueante `StorePermissionGate` (write-class pede aprovação, parqueia o run), store `PendingPermissions`, as 3 listas lêem o store, `permission.respond` (V1) + `v2.session.permission.reply` (V2) acordam o run. O **CRUD de sessão** persiste no DB (#125–#127). Restam **ações/extensões que usam a engine** (abaixo).
2. **Ações de sessão** (usam a engine, têm escolhas de semântica → confirmar). ✅ **Fundação `Message`/`Part` V1 modelada** (#138 — os 40 tipos + `session.message` wired-404). Restam os **consumidores** que precisam de projeção/run + persistência V1: `session.command` (run síncrono → `{info, parts}`), `session.shell` (idem + 409), `session.messages` (GET lista `[{info,parts}]`), `session.deleteMessage`, `part.update`/`part.delete` (precisam de um **store de mensagem V1** + projeção do turno → `Message`/`Part`; hoje o runner projeta só o timeline V2 `session_message`). `session.summarize`/`compact` (semântica de `compaction.ts`: prompt de sumário → drive → `CompactionPart` → context-prep). `session.fork` (cópia de histórico). ✅ `session.init`/`prompt_async` já feitos (#135/#137). ✅ Question flow fim-a-fim (#133–#134).
