...
>
> ⚠️ **Achado que reescreve a Fase 6:** `core` NÃO é um backend isolado — é a **base compartilhada do monorepo** (util/flag/global/installation importados em RUNTIME por `app`/`ui`/`web`/`tui`/`enterprise`/`opencode`-CLI; em `app` são 51 imports, 0 type-only). Deletar `core/server/llm` quebra as frontends (que ficam TS) e o CLI. **Plano faseado p/ chegar a "sem TS de backend" com frontend web:** (1) ✅ CORS; (2) extrair `@opencode-ai/util` (mover util/flag/global/installation do core) e repontar os **sobreviventes** `app`/`ui`/`web`; (3) `packages/app` consumir 100% do backend Rust via SDK (verificar build/e2e — fora deste ambiente); (4) remover `tui`/`desktop`/CLI-host (`packages/opencode`) + então deletar os módulos puramente-backend de `core/server/llm` + reapontar publish/CI. Etapas 3–4 precisam de verificação de build das frontends (não roda aqui) → faço por fatias revisáveis.
>
