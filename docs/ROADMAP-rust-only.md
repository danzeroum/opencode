...
>
> **Rotas nativas contrato-enforçadas: 44 paths** · PRs desta rodada autônoma: #69–#106.
...
- ✅ 1v — `v2.fs.list` (`GET /api/fs/list`, dados reais: `{ path, type, mime }` via `opencode_tools`, mime best-effort sem dep nova) — **#106**; restam `v2.fs.find` (busca) e `v2.fs.read` (path wildcard + corpo raw)
