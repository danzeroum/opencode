...
>
> **Última atualização:** 2026-06-27 · **Decisão do dono:** **cutover TOTAL (alvo B)**; **frontend será web** (`packages/app`, SPA SolidJS via `@opencode-ai/sdk` sobre HTTP). **Backend: 168/168 (100%) nativo, gate ESTRITO verde, Rust-only por padrão (#170).** Agora com **CORS** no servidor Rust (mesma allow-list do `cors.ts`) p/ a SPA chamar a API cross-origin.
>
