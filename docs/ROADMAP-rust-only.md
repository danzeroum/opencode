...
>
> **Última atualização:** 2026-06-27 · **Decisão do dono:** **cutover TOTAL (alvo B)**; **frontend será web** (`packages/app`). **Backend: 168/168 (100%) nativo, Rust-only por padrão, CORS + serve a SPA. 🔥 TS BACKEND DELETADO (stage 3b):** apagados `core`/`server`/`llm`/`tui`/`desktop`/`cli` + infra (~1934 arq.); `packages/opencode` virou wrapper (`bin/` + package.json). Sobreviventes: `app`/`ui`/`web`/`enterprise`/`slack`/`sdk`/`plugin`/`util`. Cassettes do LLM movidas p/ `crates/opencode-llm/fixtures/`. **✅ Mergeado (#174) com CI VERDE (`rust` + `unit`/bun-turbo-test)** — a deleção é sólida (workspace instala, testes dos sobreviventes passam). **ALVO B FUNCIONALMENTE COMPLETO: o backend é 100% Rust, o TS de backend não existe mais.** Resta só **stage 4** (release-eng, gated off no fork): embutir `app/dist` no binário (cargo-dist/rust-embed) + reapontar `publish.yml` (roda só em `anomalyco/opencode`) + limpar refs órfãs em `script/*.ts`. **Verificação do dono (não roda aqui):** `bun --cwd packages/app build` + smoke da SPA servida (`OPENCODE_WEB_DIR=packages/app/dist` + binário Rust).
>
> ⚠️ **Achado que reescreve a Fase 6:** `core` NÃO é um backend isolado — é a **base compartilhada do monorepo** (util/flag/global/installation importados em RUNTIME por `app`/`ui`/`web`/`tui`/`enterprise`/`opencode`-CLI; em `app` são 51 imports, 0 type-only). Deletar `core/server/llm` quebra as frontends (que ficam TS) e o CLI. **Plano faseado (defaults do dono: launcher fino, apaga tui/desktop, mantém web/enterprise/slack, `app` servida pelo binário Rust):** (1) ✅ CORS; (2) ✅ extraído `@opencode-ai/util` (8 módulos puros encode/path/binary/retry/iife/array/lazy/error) + repontados `app`/`ui`/`web`/`enterprise` (49 arq.) → **frontend web 0 imports de `core`**; (3) deletar `core`/`server`/`llm`/`tui`/`desktop`/`cli` + reescrever `packages/opencode` como launcher fino (sobe o binário Rust); (4) `app` servida pelo binário Rust + reapontar publish/CI. Verificação TS via CI `unit` (bun turbo test); o build/e2e final da SPA confirma o dono (não roda aqui).
>
