...
>
> **Rotas nativas contrato-enforçadas: 61 paths** (várias com múltiplos métodos) · PRs desta rodada autônoma: #69–#134. **Engine**: runner loop completo + permission gating real (#129–#131) + **question flow fim-a-fim** (store/rotas #133 + tool produtor `question` #134). CI `rust` verde (#119). **Loading real**: `command`/`skill`/`agent` lêem `.opencode/**.md`. **Escrita real**: session CRUD (`/session`, `/session/{id}`) persiste no DB (#125–#127). **Engine — permission gating completo**: gate bloqueante `StorePermissionGate` (write-class pede aprovação) + store de pendências + listas + `permission.respond` resolve e acorda o run (#129–#130).
...
2. **Ações de sessão** (usam a engine, têm escolhas de semântica → confirmar): `session.command` (retorna `{info, parts}` — precisa modelar `Message`/`Part` V1 + run síncrono), `session.summarize`/`compact` (sumariza/compacta o contexto), `session.init`/`fork`/`shell`/`prompt_async`. **Question flow**: `v2.session.question.reply`/`reject` + um **tool produtor** (`question`/`ask`) que parqueia no store (espelha o permission gate).
